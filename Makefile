...
build:
	docker compose run --rm dev go build -ldflags="$(LDFLAGS)" -o bin/verify-loop .
...
BINARY := bin/verify-loop$(EXT)

ifeq ($(GOOS),windows)
  INSTALL_DIR ?= $(LOCALAPPDATA)/Programs/verify-loop
else
...
install:
	...
	@mkdir -p "$(INSTALL_DIR)"
	cp $(BINARY) "$(INSTALL_DIR)/verify-loop$(EXT)"
	@echo "installed verify-loop $(VERSION) ($(GOOS)/$(GOARCH)) to $(INSTALL_DIR)/verify-loop$(EXT)"
...
cross:
	docker compose run --rm dev sh -c "\
		CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/verify-loop-linux-amd64 . && \
		CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/verify-loop-darwin-amd64 . && \
		CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags='$(LDFLAGS)' -o bin/verify-loop-darwin-arm64 . && \
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags='$(LDFLAGS)' -o bin/verify-loop-windows-amd64.exe ."
